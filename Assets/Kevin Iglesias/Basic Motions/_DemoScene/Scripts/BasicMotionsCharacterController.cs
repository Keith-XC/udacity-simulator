...
namespace KevinIglesias
{
    ...
    public class BasicMotionsCharacterController : MonoBehaviour
    {
        ...
        private MoveAlongCircuit moveAlongCircuit;
        ...
        private void Awake()
        {
            try
            {
                moveAlongCircuit = GetComponent<MoveAlongCircuit>();
                ...
                Vector3 origin = transform.position + (Vector3.up * collisionBox.center.y) +
                                 (-Vector3.up * (collisionBox.size.y * 0.5f));
                distanceToGround =
                    (origin.y - transform.position.y) * 1.01f; //1.01f = MARGIN TO MAKE SURE RAYS TOUCH GROUND
        ...
        private void Update()
        {
            // Wenn MoveAlongCircuit vorhanden ist und dessen Speed 0 ist, dann soll keine Bewegungsanimation abgespielt werden.
            if (moveAlongCircuit != null && moveAlongCircuit.Speed == 0f)
            {
                // Setze sämtliche Inputwerte auf Null, damit der Charakter in Idle übergeht
                inputX = 0f;
                inputY = 0f;

                // Setze die booleschen Variablen für Bewegung auf false
                animator[0].SetBool("Moving", false);
                animator[1].SetBool("Moving", false);

                // Optional: Wechsel den Charakter State, falls benötigt
                if (characterState == CharacterState.Moving)
                {
                    ChangeState(CharacterState.Idle);
                }

                // Wir wollen an dieser Stelle keinen normalen Input mehr verarbeiten, also können wir hier "return" aufrufen
                // damit der Rest der Eingabe-Logik in diesem Frame nicht ausgeführt wird.
                return;
            }

            // Wenn der MoveAlongCircuit entweder nicht vorhanden ist oder Speed nicht 0 ist, dann normale Inputs lesen
            GetInputs();
            ControlCharacter();
        }
